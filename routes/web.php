...
// Emergency test route - bypasses everything, runs before any middleware
...
// Simple test routes - these go through web middleware but should still work
...
Route::group(['prefix' => 'admin', 'namespace' => 'Admin', 'middleware' => ['guest:admin']], function(){
	...
	Route::group(['middleware' => 'permission:manage_pages'], function () {
		Route::get('pages', 'PagesController@index'); //admin/pages
		Route::match(array('GET', 'POST'), 'add-page', 'PagesController@add');
		Route::match(array('GET', 'POST'), 'edit-page/{id}', 'PagesController@update');
		Route::get('edit-page-ajax/{id}', 'PagesController@editPageAjax')->name('adminEditPageAjax'); //admin/edit-page-ajax/id
	...
	Route::group(['middleware' => 'permission:general_setting'], function () {
		...
		Route::get('settings/system', 'SettingsController@system')->middleware(['permission:general_setting']);
...
Route::group(['middleware' => ['no_auth:users', 'locale']], function () {
    ...
    Route::post('users/veriff-complete', 'UserController@identificationVeriffComplete');
    
    Route::post('users/veriff-process', 'UserController@identificationVeriffProcess');
});
...
Route::group(['middleware' => ['guest:users', 'locale']], function () {
    ...
    Route::match(array('GET', 'POST'),'listing/{id}/{step}', 'PropertyController@listing')->where(['id' => '[0-9]+','page' => 'basics|description|external_link|location|amenities|photos|pricing|calendar|details|booking||cancellation_policy|payment_processing']);
    ...
    Route::post('messaging/reply/', 'InboxController@messageReply'); //text only
    Route::post('messaging/upload-file/', 'InboxController@uploadFile')->name('messaging.upload-file'); //files only
