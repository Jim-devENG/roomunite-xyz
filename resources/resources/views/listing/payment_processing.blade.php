			...
			<div class="col-md-10">
				<div class="main-panel min-height mt-4">
					<div class="row justify-content-center">
					      <div class="col-md-10">
                	       <h1 class="font-weight-700">{{ $result->name }}</h1>
                	     </div>
							...
							<form id="booking_id" method="post" action="{{url('listing/'.$result->id.'/'.$step)}}"  accept-charset='UTF-8'>
								...
								<div class="col-md-12 p-0 mt-4 border rounded pb-4 m-0">
									<div class="form-group col-md-12 main-panelbg pb-3 pt-3 pl-4">
											<h4 class="text-16 font-weight-700">{{trans('messages.listing_sidebar.payment_processing')}}</h4>
									</div>
									<div class="row m-0 pl-5 pr-5">
										<div class="col-md-12 p-0">
											<h3>{{trans('messages.payment_processing_fees.title')}} <span class="text-danger">*</span></h3>
											<p class="text-muted">{{trans('messages.payment_processing_fees.label')}}.</p>
										</div>
									</div>
									
									<div class="row m-0">
										<div class="col-md-4 pl-5 pr-5">
											<label>{{trans('messages.listing_sidebar.payment_processing')}}</label>
											<select name="booking_type" id="booking_type" class="form-control text-16 mt-2">
												<option value=0 {{ ($result->payment_processing == 0) ? 'selected' : '' }}>{{trans('messages.payment_processing_fees.yes_option')}}</option>
												<option value=1 {{ ($result->payment_processing == 1) ? 'selected' : '' }}>{{trans('messages.payment_processing_fees.no_option')}}</option>
											</select>
										</div>
									</div>
								</div>
								...
								<div class="col-md-12 mt-4 p-0 mb-5">
									<div class="row justify-content-between">
										<div class="mt-4">
											<a href="{{ url('listing/'.$result->id.'/booking') }}" class="btn btn-outline-danger secondary-text-color-hover text-16 font-weight-700 pl-5 pr-5 pt-3 pb-3">
