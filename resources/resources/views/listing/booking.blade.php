							...
							<form id="booking_id" method="post" action="{{url('listing/'.$result->id.'/'.$step)}}"  accept-charset='UTF-8'>
								...
								<div class="col-md-12 p-0 mt-4 border rounded pb-4 m-0">
									...
									<div class="row mt-2">
											...
											<label>Minimum days per booking</label>
												<input type="number" name="min_days" id="min_days" class="form-control text-16 mt-2" value="{{ old('min_days', $result->min_days)  }}" placeholder="Min. Days of Stay" maxlength="30">
										<span class="text-danger">{{ $errors->first('min_days') }}</span>
												
											
										</div>
									    	<div class="col-md-6 pl-5 mt-4">
											<label>Maximum days per booking</label>
												<input type="number" name="max_days" id="max_days" class="form-control text-16 mt-2" value="{{ old('max_days', $result->max_days)  }}" placeholder="Maximum days is 30" maxlength="30">
