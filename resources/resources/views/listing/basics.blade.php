					...
					<div class="col-md-9  mt-4 mt-sm-0 pl-4 pr-4">
						<form method="post" action="{{url('listing/'.$result->id.'/'.$step)}}"  accept-charset='UTF-8' id="listing_bes">
							...
							<div class="form-row mt-4 border rounded pb-4">
								...
								<div class="form-group col-md-6 pl-5 pr-5">
    								<label for="furnshing">{{trans('messages.property.furnishing')}}</label>
    								<select name="furnshing" id="furnshing" class="form-control text-16">
    								    @foreach($furnishing_type as $key => $value)
        									<option value="{{ $key }}" {{ ($key == $result->furnishing_status) ? 'selected' : '' }}>
        									    {{ $value }}
        									</option>
    									@endforeach
    								</select>
    							...
    							<div class="form-group col-md-6 pl-5 pr-5">
    								...
    								<select name="stay_term" id="stay_term" class="form-control text-16">
    								    @foreach($stay_duration_term as $key => $value)
        									<option value="{{ $key }}" {{ ($key == $result->stay_duration_type) ? 'selected' : '' }}>
        									    {{ $value }}
        									</option>
    									@endforeach
    								</select>
