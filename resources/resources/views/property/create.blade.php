...
<div class="mb-4 margin-top-85">
	<div class="row m-0">
		...
		<div class="col-md-10  min-height">
			<div class="main-panel m-4 list-background border rounded-3 pb-4">
				<h3 class="text-center mt-5 text-24 font-weight-700">{{trans('messages.property.list_space')}}</h3>
				<p class="text-center text-16 pl-4 pr-4">{{ $site_name ?? 'RoomUnite' }} {{trans('messages.property.property_title')}}.</p>
				...
				@if(Auth::user()->verified > 0)
    				<form id="list_space" method="post" action="{{url('property/create')}}" class="mt-4" id="lys_form" accept-charset='UTF-8'>  
    					...
    					<div class="row p-4">
    						...
    						<div class="col-md-6">
    							<div class="form-group mt-4">
    								<label for="furnishing">{{trans('messages.property.furnishing')}}</label>
    								<select name="furnishing" id="furnishing" class="form-control text-16">
    								
    									<option class="accommodates" value="0">
    									    Furnished
    									</option>
    								...
    								@if ($errors->has('furnshing')) <p class="error-tag">{{ $errors->first('furnshing') }}</p> @endif
    							</div>
    						</div>
    						
    						<div class="col-md-6">
    							<div class="form-group mt-4">
    								<label for="stay_term">{{trans('messages.property.stay_duration')}}</label>
    								<select name="stay_term" id="stay_term" class="form-control text-16">
    								
    									<option class="accommodates" value="short">
    									    Less than 6 months
    									</option>
    									<option class="accommodates" value="long">
    									    6 months and above
    									</option>
    									
    								</select>
    								@if ($errors->has('stay_term')) <p class="error-tag">{{ $errors->first('stay_term') }}</p> @endif
    							</div>
    						</div>
