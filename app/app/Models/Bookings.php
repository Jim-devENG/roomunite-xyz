...
class Bookings extends Model
{
    ...
    {
        // Handle legacy databases where users table may not have first_name/last_name
        $hostNameColumn  = \Schema::hasColumn('users', 'first_name') ? 'first_name' : (\Schema::hasColumn('users', 'name') ? 'name' : null);
        $guestNameColumn = \Schema::hasColumn('users', 'first_name') ? 'first_name' : (\Schema::hasColumn('users', 'name') ? 'name' : null);

        $hostNameSelect  = $hostNameColumn
            ? \DB::raw("`u`.`{$hostNameColumn}` as host_name")
            : \DB::raw("NULL as host_name");

        $guestNameSelect = $guestNameColumn
            ? \DB::raw("`users`.`{$guestNameColumn}` as guest_name")
        ...
        $bookings = Bookings::join('properties', function ($join) {
                        ...
                        ->select([
                            ...
                            \DB::raw('CONCAT(bookings.total) AS total_amount'),
                            'bookings.status',
