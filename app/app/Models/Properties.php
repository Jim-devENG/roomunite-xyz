...
class Properties extends Model
{
    ...
    public static function recommendedHome()
    {
        $data = parent::where('status', 'listed')
                        ->with('users','property_price.currency', 'property_address')
                        ->where('recomended', '1')
                        ->take(32)
                        ->inRandomOrder()
                        ->get();
    ...
    public function getHostNameAttribute()
    {
        try {
            ...
            \Log::warning('Failed to get host name for property ' . $this->attributes['id'] . ': ' . $e->getMessage());
            return 'N/A';
        }
    }
    ...
    public function getPropertyTypeNameAttribute()
    {
        try {
            ...
            return $propertyType ? $propertyType->name : 'N/A';
        } catch (\Exception $e) {
            \Log::warning('Failed to get property type name: ' . $e->getMessage());
            return 'N/A';
        }
    }
    ...
    public function getSpaceTypeNameAttribute()
    {
        try {
            ...
            return $spaceType ? $spaceType->name : 'N/A';
        } catch (\Exception $e) {
            \Log::warning('Failed to get space type name: ' . $e->getMessage());
            return 'N/A';
        }
    }
    ...
    public function getCoverPhotoAttribute()
    {
        try {
            // Use eager loaded relationship if available
            if ($this->relationLoaded('property_photos')) {
                $cover = $this->property_photos->where('cover_photo', 1)->first();
            } else {
                ...
                try {
                    ...
                    // If query fails or times out, return default
                    return asset('images/default-image.png');
                }
            }
            if (isset($cover->photo)) {
                $url = asset('images/property/'.$this->attributes['id'].'/'.$cover->photo);
            } else {
                $url = asset('images/default-image.png');
            }
            return $url;
        } catch (\Exception $e) {
            \Log::warning('Failed to get cover photo for property ' . ($this->attributes['id'] ?? 'unknown') . ': ' . $e->getMessage());
            return asset('images/default-image.png');
        }
    }
