...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $email_controller->booking(1, setDateForDb('2024-04-17'));
        $email_controller->booking_user(1, '2024-04-17');
    ...
    public function createBooking(Request $request)
    {
        ...
        //$amount             = round($this->helper->convert_currency($request->currency, $currencyDefault->code, $price_list->total),2);
         ...
         $amount             = round($this->helper->convert_currency($request->currency, $currencyDefault->code, $price_list->weekly_total_price),2);
    ...
    public function stripeRequest(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (isset($request->stripeToken)) {
                ...
                $price_eur     = $this->helper->convert_currency($this->helper->getCurrentCurrencyCode(), $currencyDefault->code, $price_list->weekly_total_price);
                
                $checkIn = Session::get('payment_checkin');
                $checkOut = Session::get('payment_checkout');
                
                $mcheckIn = setDateForDb($checkIn);
                $mcheckOut = setDateForDb($checkOut);
                
                $from = new DateTime($mcheckIn);
                $to = new DateTime($mcheckOut);
    ...
    private function getGoogleAccessToken(){
        ...
        //'the-folder-and-filename-of-your-downloaded-service-account-file.json'
         $credentialsFilePath = storage_path('app/json/appastorenew-firebase-adminsdk-4sctg-ef1872464b.json'); //replace this with your actual path and file name
    ...
    public function sendNotificationMessage(Object $request){

        $apiurl = 'https://fcm.googleapis.com/v1/projects/appastorenew/messages:send';   //replace "your-project-id" with...your project ID
        
        Log::info("RETURN TOKEN VALUE: ");
        Log::info($this->getGoogleAccessToken());
        ...
        $in_app_module = [
            ...
            "body"           => $request->body,
            "type"           => $request->type,
            "order_id"       => $str_orderid,
            "image"          => "2021-06-30-60dc3465e071f.png"
        ];
        ...
        //'token'            => "cprCZKFeSB-XO33Qwdea8m:APA91bEkgPWCavpmqZ6goSGge7jbgrGg-PHyGc6uTEdsQtoT0NJaBgcBS_VswMAR_fUWbq0SQBU3PGqAk7PR18WPaO2Wjsy4wGyWKMfgf_jPRg1Q6W-5xIe0MIFdFCorzBXd-8LtqF_J",
    ...
    public function sendMultipleNotificationMessage(Object $request){
       ...
       $str_orderid = (string)$request->order_id; 
