...
class BookingController extends Controller
{
    ...
    */
    public function requestPayment(Request $request){
        ...
        Session::put('payment_booking_nights', $data['nights']);
        ...
        if (((isset($data['price_list']->status) && ! empty($data['price_list']->status)) ? $data['price_list']->status : '') == 'Not available') {
            ...
            Session::forget('payment_booking_nights');
    ...
    */
    ...
    public function guestCancel(Request $request)
    {
        ...
        // Check if cancellation is requested before the booking starts
        if ($now < $bookingStart) {

            // Logic for handling payouts and penalties, if applicable for guest cancellations
            ...
            // Save cancellation reason in booking details, if necessary
            $bookingDetails = new BookingDetails;
            $bookingDetails->booking_id = $request->id;
            $bookingDetails->field = 'cancelled_reason';
            $bookingDetails->value = $request->cancel_reason;
            $bookingDetails->save();

            // Send email notification and display success message
            // $email->bookingCancellation($request->id);
            $this->helper->one_time_message('success', trans('messages.success.resere_cancel_success'));
            // clearCache('.calc.property_price');
            ...
            // Display error message if cancellation is not allowed
            $this->helper->one_time_message('danger', trans('messages.error.you_cant_cancel_booking_now'));
            return redirect('my-bookings');
        }
    }
