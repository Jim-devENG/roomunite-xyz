...
class PropertiesController extends Controller
{
    ...
    public function index2(PropertyDataTable $dataTable)
    {
        // return $query = Properties::with(['users:id,first_name,profile_image']);
        ...
        return $dataTable->render('admin.properties.viewNew', $data);
    }

    public function index(PropertyDataTable $dataTable)
    {
        // return $properties = Properties::where(['recomended'=>1])->count();
        $data['from'] = isset(request()->from) ? request()->from : null;
        $data['to']   = isset(request()->to) ? request()->to : null;
        $data['space_type_all'] = SpaceType::getAll();
    ...
    {
        // // Assuming $property contains the details of the newly created property
        $property = [
        ...
        // Admin email address
        $hostEmail = 'support@roomunite.com';
        ...
        $data = [
            'property' => $property, // Pass the property details to the email template
        ];

        // Send the email to the admin
        Mail::send('emails.new_property_template', $data, function($message) use ($hostEmail, $subject) {
    ...
    public function listing(Request $request, CalendarController $calendar)
    {
        ...
        if ($step == 'basics') {
            if ($request->isMethod('post')) {
                ...
                if (($property->status=='Unlisted') && ($request->list_status=='Listed')) {
                    // Send Mail to Admin
                    $property = [
                        'title' => $property->name,
                    ...
                    // Admin email address
                    $host = User::where('id', $property->host_id)->first();
                    ...
                    // Send the email to the admin
                    Mail::send('emails.property_list_success', $data, function($message) use ($hostEmail, $subject) {
                        $message->to($hostEmail)->subject($subject);
                    });
                ...
                $property->status             = $request->list_status;
                $property->save();
