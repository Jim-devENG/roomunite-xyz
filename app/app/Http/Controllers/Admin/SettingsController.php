...
class SettingsController extends Controller
{
    ...
    public function fees(Request $request)
    {
        if ($request->isMethod('post')) {
            $rules = array(
                    ...
                    'damages_insurance'     => 'required|numeric|max:99|min:0',
                    'payment_processing_fee'=> 'required|numeric|max:99|min:0',
            ...
            $fieldNames = array(
                    ...
                    'damages_insurance'      => 'Damages Insurance',
            ...
            if ($validator->fails()) {
                ...
                PropertyFees::where(['field' => 'damages_insurance'])->update(['value' => $request->damages_insurance]);
                PropertyFees::where(['field' => 'payment_processing_fee'])->update(['value' => $request->payment_processing_fee]);
  ...
  public function n_as_k_c() {
        // Bypass purchase code verification for local/development environments
        if (in_array(config('app.env'), ['local', 'development'])) {
            return false; // Allow access in local/dev
        }
        if(!g_e_v()) {
            return true;
        }
    ...
    public function system(Request $request)
    {
        ...
        $data['server_software'] = isset($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : 'Unknown';
        $data['php_os'] = PHP_OS;
        $data['server_name'] = isset($_SERVER['SERVER_NAME']) ? $_SERVER['SERVER_NAME'] : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'Unknown');
        $data['document_root'] = isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : base_path('public');
        $data['app_env'] = env('APP_ENV', 'Unknown');
        $data['app_debug'] = env('APP_DEBUG', false) ? 'Yes' : 'No';
        $data['db_connection'] = config('database.default', 'Unknown');
        $data['cache_driver'] = config('cache.default', 'Unknown');
        $data['session_driver'] = config('session.driver', 'Unknown');
